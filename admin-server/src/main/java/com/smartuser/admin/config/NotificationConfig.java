...
public class NotificationConfig {
	...
	public NotificationConfig(InstanceRepository instanceRepository, MailSender mailSender,
	...
	public DownOnlyMailNotifier downOnlyMailNotifier(MailSender mailSender, InstanceRepository repository,
	 ...
	 */
	@Scheduled(fixedDelayString = "${spring.boot.admin.notify.mail.component-check-interval:30000}") // Default 30
																										// seconds
	...
	public void checkComponentHealth() {
		...
		instanceRepository.findAll().collectList().doOnNext(instances -> {
			instances.forEach(instance -> {
				try {
					String healthUrl = instance.getRegistration().getManagementUrl() + "/actuator/health";
					String healthJson = webClient.get().uri(healthUrl).retrieve().bodyToMono(String.class).block();
		...
		}).subscribe(instances -> log.trace("Checked health for {} instances", instances.size()),
	...
	private void checkComponentStatus(Instance instance, String healthJson, MailSender mailSender, String to,
		...
		try {
			...
			if (components != null) {
				...
				Map<String, String> previousStatuses = previousComponentStatuses.computeIfAbsent(instanceId,
						k -> new HashMap<>());
	...
	private void sendComponentEmail(Instance instance, String componentName, String status, JsonNode componentDetails,
			MailSender mailSender, String to, String from, String subject) {
		try {
			...
			log.info("Email notification sent for component {} in service {} ({}) with status {}", componentName,
					instance.getRegistration().getName(), instance.getId(), status);
		} catch (Exception e) {
			log.error("Failed to send email notification for component {} in service {} ({})", componentName,
					instance.getRegistration().getName(), instance.getId(), e);
		}
	}

	private String buildComponentEmailBody(Instance instance, String componentName, String status,
			JsonNode componentDetails) {
		...
		body.append("\n");
		body.append("The ").append(componentName.toUpperCase())
				.append(" component has gone DOWN. Please check immediately.\n");
		body.append("\n");
	...
	public static class DownOnlyMailNotifier extends AbstractStatusChangeNotifier {
		...
		private void sendEmail(Instance instance, String status) {
			try {
				...
				log.error("Failed to send email notification for service {} ({})", instance.getRegistration().getName(),
						instance.getId(), e);
			}
		}
